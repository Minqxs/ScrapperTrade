...
public sealed class PortfolioRiskEngine {
 ...
 public RiskDecision Evaluate(CandidateTrade t,SymbolMetadata s,PortfolioSnapshot x,string group,DateTimeOffset now)=>Evaluate(t,s,x,group,now,new());
 public RiskDecision Evaluate(CandidateTrade t,SymbolMetadata s,PortfolioSnapshot x,string group,DateTimeOffset now,InstrumentRiskPermissions permissions){
  if(!permissions.Enabled)return No("INSTRUMENT_DISABLED","Instrument permission is disabled.");
  if(!permissions.Allows(t.Side))return No("DIRECTION_BLOCKED",$"{t.Side} entries are not permitted.");
  if(!permissions.IsSessionOpen(now))return No("SESSION_CLOSED","Instrument trading session is closed.");
  var recent=x.RecentOrders??[];
  if(recent.Any(z=>z.SignalId==t.SignalId))return No("DUPLICATE_SIGNAL","Signal was already submitted.");
  if(recent.Count(z=>now-z.SubmittedAt>=TimeSpan.Zero&&now-z.SubmittedAt<=p.FrequencyWindow)>=p.MaxOrdersPerWindow)return No("ORDER_FREQUENCY","Order frequency limit reached.");
  ...
  if(x.Positions.Where(z=>z.ExposureGroup==group).Sum(z=>z.RiskAmount)+size.ActualRisk>x.Equity*p.MaxGroupRiskFraction)return No("GROUP_RISK","Exposure group limit exceeded.");
  if(x.Positions.Where(z=>z.ExposureGroup==group&&z.Side==t.Side).Sum(z=>z.RiskAmount)+size.ActualRisk>x.Equity*p.MaxCorrelatedDirectionRiskFraction)return No("CORRELATED_EXPOSURE","Same-direction correlated exposure limit exceeded."); return new(true,"APPROVED","Hard risk policy passed.",size.Volume,size.ActualRisk);
